...
final class MetaSchemaValidator {
    ...
    void validateMetaSchema(JsonParser jsonParser, String metaSchemaUri, String schemaUri, JsonNode node) {
        ...
        Schema schema = resolveMetaSchema(jsonParser, metaSchemaUri)
                .orElseThrow(() -> new MetaSchemaResolvingException("Cannot resolve meta-schema [%s]".formatted(metaSchemaUri)));
    ...
    private Optional<Schema> resolveExternalSchema(JsonParser jsonParser, String uri) {
        ...
        if (schemaRegistry.get(baseUri) != null) {
            throw new MetaSchemaResolvingException("Cannot resolve meta-schema [%s]".formatted(uri));
        }
        String rawJson = schemaResolver.resolve(baseUri).orElseThrow(() -> new MetaSchemaResolvingException("Cannot resolve meta-schema [%s]".formatted(uri)));
        try {
